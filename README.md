...
**ARGUS** (ATLAS Resource Guided User System) is the operator console. Named after Argus Panoptes, the hundred-eyed watchman, ARGUS exists to make ATLAS's distributed state legible and actionable. A federated workflow is not one event — it is many asynchronous truths happening at once across the ATLAS network: cohort composition, artifact generation, model state, transfer status, and execution provenance. ARGUS functions as the watchman over ATLAS, guiding the operator through the SeaGaP-MP stage sequence (Search, Gather, Process, Monitor, Post) that structures all federated interaction.

**CALYPSO** (**C**ognitive **A**lgorithms & **L**ogic **Y**ielding **P**redictive **S**cientific **O**utcomes) is the AI intent layer — the conversational mediator between human intent and deterministic execution. As the **daughter of ATLAS**, CALYPSO is born from the infrastructure she serves, acting as the intelligent bridge that conceals the platform's underlying complexity for the user. In myth, Calypso is associated with concealment; the project inverts that purpose. CALYPSO does not conceal outcomes; CALYPSO conceals unnecessary friction. The user speaks in intent, the system executes deterministically, and the evidence is left behind as materialized state. CALYPSO is shared across browser and CLI surfaces: the same operational core drives both, so guidance is consistent regardless of how the user connects.

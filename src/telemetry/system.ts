...
export function processList_render(isProvisioning: boolean): void {
    ...
    if (isProvisioning) {
        ...
        const step: string = steps[Math.floor(Math.random() * steps.length)];
        ...
        // Sort by CPU
        procs.sort((a: ProcessInfo, b: ProcessInfo) => parseFloat(b.cpu) - parseFloat(a.cpu));
        
        let html: string = '<span class="dim">  PID USER     %CPU %MEM COMMAND</span>\n';
        procs.forEach((p: ProcessInfo) => {
...
export function networkStats_render(): void {
    ...
    const eth0_rx: string = (42 + state.cycle * 0.1 + Math.random()).toFixed(2);
    const eth0_tx: string = (12 + state.cycle * 0.05 + Math.random()).toFixed(2);

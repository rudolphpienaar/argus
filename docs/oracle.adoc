...
Supported step keys in the current runner:

1. `send` - execute one Calypso command string.
2. `success` - expected boolean for the previous `send`.
3. `output_contains` - string or string[] that must appear in the response message.
4. `capture_project` - when `true`, stores active project name into `${project}`.
5. `vfs_exists` - asserts path existence after interpolation (`${user}`, `${project}`).

Reference scenario: `tests/oracle/fedml-smoke.oracle.json`.

...
*   **Lifecycle:** Each stage module (e.g., `src/core/stages/process.ts`) exports `onEnter()` and `onExit()` hooks.
*   **Control Flow:** The orchestrator calls `currentStage.onExit()` -> `nextStage.onEnter()`.

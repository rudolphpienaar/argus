...
----
assetDetail_open(id)
├── overlay.dataset.mode = 'marketplace'   (no slots needed)
├── detailHeader_populate(...)
├── detailSpecs_populate(...)
├── detailContent_populate(...)
└── overlay.classList.remove('hidden')

assetDetail_close()
├── overlay.classList.add('closing')
└── on animationend → overlay.classList.add('hidden')
----

==== Project Detail → Workspace

----
projectDetail_open(id)
├── projectDetail_populate(...)
│   ├── overlay.dataset.mode = 'project'
│   ├── write sidebar → #overlay-sidebar-slot
│   ├── write content → #overlay-content-slot
│   ├── write commands → #overlay-command-slot
│   └── instantiate FileBrowser
└── overlay.classList.remove('hidden')

workspace_expand(id, project)
...
projectDetail_close()
├── workspace_collapse()  [if expanded]
...
==== Dataset Detail (Gather Mode)

----
datasetDetail_open(id)
├── datasetDetail_populate(...)
│   ├── overlay.dataset.mode = 'dataset'
│   ├── write sidebar → #overlay-sidebar-slot
│   ├── write content → #overlay-content-slot  (selectable FileBrowser)
│   ├── write commands → #overlay-command-slot  (DONE / ADD / CANCEL)
│   └── mount temp VFS tree
└── overlay.classList.remove('hidden')

datasetDetail_close()
├── datasetBrowser.destroy()
├── unmount temp VFS tree
├── overlaySlots_clear()
├── overlay.dataset.mode = 'marketplace'
└── slide-out animation → hidden
----
...
== Window Binding Convention

Functions that need to be callable from HTML `onclick` attributes must be:

1. **Declared** on the global `Window` interface:
+
[source,typescript]
----
declare global {
...
2. **Assigned** in the initialization function:
+
[source,typescript]
----
...
3. **Registered** in `argus.ts` (the main entry point) if the function is exported from a stage module.

This replaces `(window as any).foo = foo` casts. The TypeScript compiler enforces type safety on the binding.
...
1. **`overlaySlots_clear()`**: Currently in `search.ts`. Should live in a shared overlay utilities module if more slot-based overlays are added.
2. **`workspace_teardown()`**: Currently in `search.ts`, imported by `process.ts`. If more stage transitions need workspace cleanup, this should move to a lifecycle module.
3. **Resize handle creation/cleanup**: Currently inline in `workspace_expand()` / `workspace_collapse()`. The pattern (create handle → attach drag listeners → return cleanup function) is generalizable.

---
_Last updated: 2026-01-30 (v4.5.0)_

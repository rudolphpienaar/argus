...
ARGUS includes a procedural LCARS frame generator for creating consistent, themeable UI components. This system enables dynamic color theming via CSS custom properties and HSL color computation.
...
The Asset Detail Overlay uses the LCARS Component System:

* Regular assets use `--lcars-hue: 200` (blue/sky)
* FDA assets use `--lcars-hue: 140` (green)
* The hue is set dynamically when opening an asset detail view

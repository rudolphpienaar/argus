...
Each section details the specific technical pressures—such as monolithic drift, instructional decay, and the irreducible hallucination of agency—that necessitated the continuous refactoring of the system's core during this high-velocity solo sprint. The architectural decisions documented here are not the result of user studies, but the expression of over two decades of practitioner expertise in scientific computing systems. By documenting the "Historical Drama" of every milestone, this specification provides the necessary context for understanding why the v10 substrate required a formal major-version contract lock in `v11.0.0` once topology, telemetry, and path semantics converged.
...
2026-01-23                                                            2026-02-23
    ...
    ├── Phase V   v9.0  .. v10.3.2   DAG + Merkle + plugin runtime hardening
    ├── Phase VI  v10.3.3.. v11.0.0  contract consolidation + major lock
    ├── Phase VII v11.0.1.. v11.0.5  IAS purity refactor + decoupled kernel
    └── Phase VIII v11.0.6.. v12.0.0  Instrumented CNS + Null Hypothesis

arc: monolithic UI prototype  ─────────────────────────▶  instrumented CNS
----
...
== Phase V: The v10.x Plugin Revolution and Hardening Line (v9.0 - v10.3.2)
_Period: February 14 – February 21, 2026_
...
=== Version 10.3.2: Documentation Closure and Operator Clarity Patch
After runtime hardening, a final gap remained between code-current behavior and
operator-facing documentation. The issue was not compute correctness; it was path
clarity. Operators and test authors still needed a single canonical reference that
showed exactly how the manifest pipeline materializes into `~/projects` and where
cross-root side effects are written.

`10.3.2` addresses that gap by publishing an explicit FEDML pipeline map (`FEDML.md`)
that enumerates each manifest stage, handler, artifact envelope, and resulting tree
shape. The release includes complete end-state trees for both renamed and non-renamed
project trajectories and codifies the external `~/searches` snapshot namespace used
by `search` stage materialization.

The result is a documentation patch that reduces operator ambiguity without changing
Host/Guest execution semantics. In practical terms, this narrows release friction:
investigation, oracle authoring, and manual verification now anchor against a single
code-aligned directory contract.

== Phase VI: Contract Consolidation and Major-Version Lock (v10.3.3 - v11.0.0)
_Period: February 21 – February 23, 2026_

=== Status Quo Before the Cut
At the close of `10.3.2`, core hardening had succeeded in the narrow sense: host
boundaries were cleaner, federation behavior had been plugin-localized, and oracle
health was restored. But a second-order instability remained. Multiple user-visible
contracts still had dual interpretations across codepaths: boot progress could appear
late or collapsed, DAG branch semantics still carried legacy rename-era assumptions
in documentation and adapters, and CLI affordances (workflow views, shell ergonomics,
conversation width) were implemented but not yet fully normalized as first-class
contract surfaces.

This was the defining catalyst for the late-10.x churn. The platform was no longer
failing through obvious crashes; it was failing through ambiguity at integration
boundaries. Those ambiguities were operationally expensive because they produced
"looks-broken" behavior under load while preserving nominal unit correctness.

=== Version 10.3.3 - 10.3.6: Boot/Login Contract Repair
The most visible failure mode was startup observability. Users experienced long
connection waits with little or no phase detail, even though boot milestones were
known and deterministic. Investigation showed the fault was not in milestone content
but in boundary timing: telemetry subscription order, prompt redraw contention in the
REPL, and mixed render ownership during early startup.

The resolution was a protocol-first boot contract formalization and renderer
extraction. `boot_log` events were treated as phase-scoped (`login_boot`,
`workflow_boot`) with monotonic sequence guarantees, and prompt exposure became a
terminal-phase gate rather than a timing heuristic. This converted startup rendering
from an emergent side effect into an explicit contract. In parallel, boot failure
handling was hardened to return deterministic hard-failure semantics rather than
falling through into partially initialized sessions.

=== Version 10.3.6 - 10.3.7: Readiness-First FEDML and UX Determinism
The second pressure front was workflow semantics. The historical branch model around
rename no longer represented the scientifically relevant gating problem. The true
risk was ill-posed ML continuation after `gather` without explicit feasibility
assessment. This drove the readiness-first FEDML redesign: `ml-readiness` moved to
immediately after `gather`, `collect` became an optional downstream reorganization,
and convergence was formalized through `join_gather_collect` with deterministic
pre-harmonization ingress.

This same window tightened user-plane determinism: shell completion regressions were
repaired, wildcard operand behavior in `ls` was corrected, `wc` was added as a
first-class builtin, and user-scoped settings introduced stable conversational-width
control in both core and `/settings` surfaces. Collectively, these changes removed
multiple operator friction points that had accumulated as collateral during rapid
refactoring.

=== The DAG Visualization Incident and Boundary Lesson
A notable late-cycle regression occurred during DAG visualization modernization. Box
rendering was moved toward Graphviz-backed layout to produce true branch/join
geometry. The initial integration introduced a local process stall under test
conditions, which exposed an important architectural lesson: deterministic UX must
include deterministic failure behavior for optional tooling dependencies. The fix
hardened the renderer with bounded execution and explicit fallback mode so the DAG
surface could fail closed without blocking workflow execution or test completion.

This incident was small in scope but structurally important. It reinforced the
project's core doctrine that presentation enhancements cannot be allowed to re-open
kernel nondeterminism.

=== Version 11.0.0: Why This Is a Major Release
The `v11.0.0` cut is justified by contract surface change, not by marketing scope.
Across late `10.3.x`, the project crossed multiple SemVer-major boundaries:
workflow topology and branch semantics changed, stage contracts moved from
rename-centric to readiness-centric convergence, startup telemetry and gating became
explicitly phased protocol behavior, and session/provenance path expectations were
reframed around deterministic portal and lineage semantics.

Treating these as patch-level events would preserve technical debt in release
language even after runtime correction. The major bump therefore functions as a
declaration that the old compatibility narrative is retired. `v11.0.0` is the first
version where documentation, manifest topology, boot protocol, and user-facing DAG
behavior are expected to describe the same system without legacy translation layers.

== Phase VII: The IAS Purity Refactor and the Decoupled Kernel (v11.0.1 - v11.0.5)
_Period: February 23, 2026_

=== Status Quo: The Post-v11 Consensus
Following the `v11.0.0` contract lock, the system had achieved topological and
protocol stability. However, a significant architectural "Shadow Debt" remained:
the `CalypsoCore` kernel was still acting as an orchestrator rather than a pure
execution engine. Domain logic—such as the specific handling of the `python`
command—and OS-level policy—such as session realignment and boot sequencing—remained
internalized. This made the kernel "smart" about the `fedml` persona but
architecturally brittle.

The engineering pressure in this phase was the absolute separation of **Intent, 
Action, and State (IAS)**. Fulfilling the project's S-Tier mandate required that 
the kernel be stripped of all domain intelligence, leaving it as a lean, 
persona-agnostic execution pipe.

=== Version 11.0.1: Compound Command Resolution and the Python Purge
Released on February 23, 2026, v11.0.1 addressed the most visible leak in the IAS
boundary. Previously, the kernel manually intercepted the `python` shell command 
to trigger stage completion, bypassing the plugin layer entirely. This was 
necessary because the `IntentParser` could only resolve single-word verbs, failing 
to recognize `python train.py` as a valid workflow intent.

The resolution was a two-part refactor. First, `IntentParser` was updated to 
support multi-word phrase matching against the manifest's declared command set. 
Second, the hardcoded `python` hack was purged from `CalypsoCore`. This allowed 
`python train.py` to route through the standard `PluginHost` to the `train` plugin, 
which then executed the command via the Shell. This move transformed the kernel 
from a domain-aware interpreter into a generic command router.

=== Version 11.0.2: The System Command Registry
v11.0.2 focused on the eviction of OS-level "Special Commands" (`/reset`, 
`/snapshot`, `/session`, etc.) from the core execution loop. These commands were 
previously handled by a massive `switch` statement in `CalypsoCore`, making the 
kernel responsible for low-level VFS and Store manipulations.

The resolution was the implementation of the **SystemCommandRegistry**. This 
release offloaded all `/` commands to specialized handlers, providing them with 
a scoped `SystemCommandContext`. This refactor ensured that the kernel remained 
"Dumb" about system-level operations, further isolating the execution pipe from 
the environment's maintenance logic.

=== Version 11.0.3: The Workflow Controller
Version 11.0.3 addressed the "Intelligence Leak" found in stage-guidance and 
confirmation handshakes. `CalypsoCore` previously owned the regex patterns for 
"what's next?" and the logic for resolving stage-local affirmative replies 
(e.g., "yes").

The resolution was the extraction of these responsibilities into the 
**WorkflowController**. By moving guidance and confirmation intelligence into a 
dedicated module, ARGUS achieved a cleaner separation between "Execution" 
(The Kernel) and "Intervention" (The Controller). The kernel now delegates all 
interactive handshakes to the controller, maintaining its role as a pure protocol 
mediator.

=== Version 11.0.4: The Federation Simulator Plugin
v11.0.4 resolved the most significant "Simulacrum Leak" in the system. The 
hardcoded timing and narrative logs for the "Phantom Federation" build and 
distribution cycle were previously driven by `setTimeout` loops inside the UI 
layer (`phases.ts`). This made the UI the "Source of Truth" for the simulation, 
violating the principle that the UI must be a passive mirror of state.

The resolution was the creation of the **Federation Simulator Plugin**. The 
narrative lore and timing logic were moved into a guest plugin, which emits a 
monotonic telemetry stream. The UI was refactored into a passive 
**FederationObserver** that listens to the `telemetryBus`. This move ensured that 
even the simulation logic is grounded in the IAS model, with the plugin driving 
the "Action" and the UI providing the "Observation."

=== Version 11.0.5: Lifecycle Orchestration and Kernel Purity
The final release of Phase VII, v11.0.5, completed the purification of 
`CalypsoCore`. The remaining lifecycle logic—specifically the interactive 
`boot()` sequence and `session_realign()` project anchoring—was evicted from the 
kernel.

The resolution was the implementation of the **BootOrchestrator** and 
**SessionManager**. These modules now own the policy for how the system starts 
and how project paths are resolved. `CalypsoCore` was reduced to a lean, 600-line 
execution kernel that simply delegates to its constituent orchestrators. This 
release marks the transition of ARGUS into a truly "Domain-Agnostic" operating 
system, where all persona-specific behavior and scientific logic reside entirely 
in Plugins and Manifests.

== Phase VIII: The Instrumented CNS and Null Hypothesis (v11.0.6 - v12.0.0)
_Period: February 23, 2026_

=== Status Quo: The Greedy Interpreter Failure
The transition to v12.0.0 was triggered by a profound conversational failure that 
exposed a fundamental flaw in the system's intelligence integration. Following the 
successful IAS refactor, we "Grounded" the LLM in the full physical manifest (RAG) 
to improve its guidance. However, this grounding backfired: because the model now 
knew the entire workflow, it became a "Greedy Interpreter." When a user asked a 
simple guidance question (e.g., "what's next?"), the model "stole" the intent and 
mapped it to a valid—but topologically locked—future command like `status`. The 
deterministic engine correctly blocked the command, but the result was a 
"Brain-Dead" system that could no longer answer basic questions.

=== Version 11.0.14 - 11.0.16: The Precedence of Truth
This failure revealed that safety in an agentic system is not a property of 
prompts, but of **Execution Physics**. We realized that deterministic filters 
*must* structurally precede probabilistic interpretation. This led to the 
**Precedence of Truth** mandate: a hardcoded interceptor pattern that ensures 
regex-based guidance and system verbs are handled by the kernel before the LLM 
ever sees the input. To enforce this, we implemented the **IntentGuard**, which 
introduced "Vocabulary Jailing"—physically blinding the AI to any command not 
currently marked as `READY` by the DAG. This effectively turned the AI into a 
"Prisoner of the DAG," ensuring it could only propose intents that were 
topologically valid.

=== Version 12.0.0: The Instrumented CNS and Source Reorganization
The final step in this arc was the formalization of the **Central Nervous System 
(CNS)**. We recognized that the "Brain" of ARGUS—the engine, the parser, the 
guard, and the status provider—was dangerously fragmented across `src/lcarslm/`. 
To ensure this critical safety stack was auditable and protected from accidental 
refactor-drift, we performed a major source tree reorganization, consolidating all 
intelligence modules into `src/lcarslm/kernel/`. This co-location created a 
centralized, high-integrity dirtree that clearly defines the "Linguistic Hardware" 
of the ship's computer.

=== Instrumentation: The Null Hypothesis and Drift Measurement
Released on February 23, 2026, v12.0.0 transforms ARGUS from a "Safe Machine" 
into a **Scientific Instrument for AI Research**. By implementing the toggleable 
**"Null Hypothesis" (Structural Bypass)**, we have created a telescope for 
measuring AI drift. In this mode, we intentionally disable all architectural 
"helps"—RAG grounding, Anaphora resolution, and FastPath matching—allowing 
researchers to observe the "Raw Light" of the model's non-determinism. This setup 
enables the quantitative calculation of the "Hallucination Gap": the mathematical 
proof of how much the ARGUS deterministic core actually improves the reliability 
of the underlying probabilistic model.
...
|**FEDML Materialization Documentation Closure**
|v10.3.2
|Operator Contract Clarity
|Publishes explicit stage-by-stage and end-state trees so runtime evidence paths remain unambiguous for testing and operations.

|**Boot/Login Phase Telemetry Contract**
|v10.3.6
|Startup Determinism
|Converts connection-time ambiguity into ordered phase evidence with prompt gating and explicit failure semantics.

|**Readiness-First FEDML Convergence**
|v10.3.7
|Scientific Gatekeeping
|Moves feasibility checks before harmonization and formalizes optional reorganization through causal join semantics.

|**Contract Lock Release**
|v11.0.0
|SemVer Boundary Clarity
|Declares topology, telemetry, and provenance contracts as major-version baseline, retiring legacy compatibility narratives.

|**IAS Purity Refactor & Decoupled Kernel**
|v11.0.5
|Orchestration Purification
|Evicts remaining domain-intelligence and lifecycle policy from the kernel, achieving a pure persona-agnostic execution substrate.

|**Instrumented CNS & Null Hypothesis**
|v12.0.0
|Intelligence Measurement
|Enables quantitative drift measurement via structural bypasses, transforming ARGUS into a scientific instrument for AI study.
|===

== Conclusion: The Arc of Integrity

The evolution of ARGUS from January through February 2026 is a chronicle of the
"Scientific Hardening" of AI. Each major version was a response to a specific
failure mode where human trust in the system was compromised by AI-mediated
ambiguity. From the early "Gutter Flash" regressions to Merkle staleness proofs and
late boot-telemetry race repair, the project has consistently prioritized
"Evidence over Assertion."

By grounding the system in a virtualized hardware substrate (VCS), a graph-based
conscience (DAG Engine), and a Merkle provenance chain, the system reached 
technical maturity in the v11.0 contract era. The `v12.0.0` release marks the 
transition from **Architecture for Safety** to **Architecture for Measurement**. 
By centralizing the CNS and providing the instrumentation to study the failures 
of the edge, ARGUS provides the definitive platform for building and measuring 

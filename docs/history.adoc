...
2026-01-23                                                            2026-02-20
    ...
    └── Phase V   v9.0  .. v10.2.2   DAG + Merkle + plugin runtime hardening
...
== Phase V: The v10.x Plugin Revolution and Hardening Line (v9.0 - v10.2.2)
...
The v10.2.1 to v10.2.2 line hardens that boundary in three concrete ways:

1.  **Plugin-Only Federation Compute:** Federation phases were materialized as explicit stage plugins (`federate-*`), removing legacy backend orchestrator pathways and state coupling (`federationState`).
2.  **Production-Pure Backend:** Synthetic compute latency was removed from backend execution paths. Any realism delay remains plugin-local and can be disabled deterministically with `CALYPSO_FAST=true` for oracle/CI throughput.
3.  **Verification Baseline Restoration:** Store/Merkle test regressions were reconciled with current runtime semantics, and oracle scenarios stabilized under fast-mode policy.

This release line is intentionally framed as a **hardening checkpoint**, not a terminal destination. `v10.2.2` marks the stabilization baseline before `v10.3` deletion work (compatibility scaffold removal) and `v11.0` contract lock (Host kernel vs plugin compute vs adapter rendering boundaries).
...
The first v10.3 wave removes three migration-era seams:

1.  **Merkle runtime mode deletion:** Removed legacy materialization mode branching and join toggle wiring. Runtime materialization now follows one canonical store/join path.
2.  **Calypso config surface reduction:** Removed runtime materialization/join env/config toggles from `CalypsoCore`, eliminating dual-mode behavior at boot.
3.  **SessionStore compatibility deletion:** Removed flat-root compatibility (`rootStageInOwnDirectory`) and corresponding compatibility tests, leaving one deterministic stage-path contract.

This release completes the architectural cleanup phase between hardening and contract
lock: fewer toggles, fewer branches, stronger invariants.
...
By grounding the entire system in a virtualized hardware substrate (VCS), a graph-based conscience (DAG Engine), and a Merkle provenance chain, ARGUS v10.2.2 provides the strongest integrity baseline yet for national-scale medical research. The project stands as a successful proof-of-concept for how agentic systems can be made structurally safe, transforming the "Hallucinating Agent" into a high-fidelity instrument of scientific discovery. The next arc is explicit: trim transitional scaffolding in v10.3 and lock contracts in v11.0.

...
As of v7.2.0, the Calypso service is split across two source trees:
...
├── federation/
│   └── FederationOrchestrator.ts   #   5-step federation handshake
├── scripts/
│   └── ScriptRuntime.ts            #   .clpso script execution
├── adapters/
│   ├── BrowserAdapter.ts           #   CalypsoCore → Terminal + DOM
│   ├── CLIAdapter.ts               #   CalypsoCore → stdout/stdin
│   ├── NodeAdapter.ts              #   CalypsoCore → test harness
│   └── StoreAdapter.ts             #   Headless store implementation
└── AIService.ts                    #   Legacy browser integration

src/calypso/                        # Service layer — protocol, transport, UI
├── protocol/
│   └── types.ts                    #   WebSocket message types + correlation IDs
├── server/
│   ├── CalypsoServer.ts            #   HTTP + WebSocket server
│   ├── RestHandler.ts              #   REST route handlers
│   └── WebSocketHandler.ts         #   Per-connection WS dispatch
├── client/
│   └── CalypsoClient.ts            #   Shared WS transport (TUI + future WUI)
├── ui/tui/
│   ├── TuiRenderer.ts              #   Spinner, colors, tables, animation
│   └── SyntaxHighlight.ts          #   ANSI syntax highlighting
└── cli/
    ├── Repl.ts                     #   WebSocket REPL loop
    └── main.ts                     #   Thin CLI entry point

src/cli/                            # Entry points (thin wrappers)
├── calypso-server.ts               #   → CalypsoServer.calypsoServer_start()
└── calypso-cli.ts                  #   Legacy HTTP CLI (deprecated)
----
...
1. **Manifest-Driven Sequencing**: Persona manifests (`*.manifest.yaml`) that define complete conversational DAGs, grounding "what comes next" the same way VFS markers ground "where am I." See `docs/CURRENT.md`.
2. **DAG Fingerprinting**: Merkle chain over the workflow DAG for staleness detection when users revisit earlier stages.
3. **Browser WUI Client**: `src/calypso/ui/web/` — browser-side WebSocket client sharing the same CalypsoClient transport as the TUI.

=== Planned

1. **Persistent Sessions**: Save/restore VFS and Store state across CLI sessions
2. **FS Materialization**: VFS → real filesystem on the CalypsoServer sandbox
3. **Script-Manifest Anchoring**: Scripts declare `manifest:` field, validated against manifest stages

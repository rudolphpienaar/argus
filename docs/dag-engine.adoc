...
The session tree materializes the DAG as a physical directory structure under `~/sessions/<persona>/session-<id>/`.
...
~/sessions/fedml/session-123/
  data/                    # Stage: search
    gather/                # Stage: gather
      data/
        gather.json        # The ArtifactEnvelope
      rename/              # Stage: rename
        data/
          _join_.../       # Synthetic Join Node
            harmonize/     # Stage: harmonize
----

The nesting encodes the DAG path. A stage is considered "Complete" if its primary artifact file exists at its topological path.

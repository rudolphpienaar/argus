...
| `phase`
| no
| Optional grouping identifier for progress display and UI breadcrumbs.
  Multiple stages can share the same phase. For example, `search`, `gather`,
  and `rename` all belong to phase `search-and-gather`. Phases provide
  high-level progress indication ("You are in the Search & Gather phase")
  while stages provide granular tracking. If omitted, the stage is its own
  group or the UI derives grouping from topology.
    ...
    rename/
      ...
      _join_gather_rename/                 ← topological join node
        data/                              ← join.json + symlinks to parent data/
          gather -> ../../../../data       ← ref to gather's artifacts
          rename -> ../../data             ← ref to rename's artifacts
        harmonize/
...
NOTE: The full FedML session tree nests 14+ levels deep (including the join
node). This is a known ergonomic concern — deeply nested directories are
awkward to navigate manually. For now this is accepted as a consequence of
the ChRIS nesting convention, and the session tree is primarily
machine-consumed (the user works in the flat project tree). Alternatives
such as hybrid nesting or phase-level grouping may be explored later.
...
An earlier design attempted to solve this with symlinks at the stage
directory level: placing a `gather` symlink inside `harmonize/` to represent
the join edge. This was wrong. The nesting convention `<parent>/<child>`
means a subdirectory is a child. A symlink to `gather` inside `harmonize`
declares "gather is a child of harmonize" — the opposite of the actual
relationship, and it introduces a cycle in the directory structure.

==== How ChRIS Handles It

In ChRIS, when a plugin takes input from multiple parents, a **topological
copy** node is inserted at the convergence point. This is a real node in the
pipeline DAG — it materializes data, it gets fingerprinted, and it appears in
the feed tree. The topological copy makes the convergence explicit as a
data-state event rather than hiding it in a structural trick.

==== How ARGUS Handles It

ARGUS follows the same principle. When a stage declares multiple parents
(`previous: [gather, rename]`), the session materializer inserts a
**topological join node** at the convergence point. The join node:

- Is named `_join_<parent1>_<parent2>` (parents listed alphabetically)
- Nests physically under the last parent in the linear chain
- Has its own `data/` directory containing:
  * A `join.json` artifact recording the convergence metadata
  * Symlinks to each parent's `data/` directory (input references)
- The downstream stage (e.g. `harmonize`) nests under the join node
...
~/sessions/fedml/session-<id>/
  gather/
    ...
    rename/
      ...
      _join_gather_rename/                 ← topological join node
        data/                              ← join artifact + parent refs
...
The join node resolves the nesting problem without introducing cycles or
false parent-child relationships:

- **No cycles.** Symlinks live inside the join node's `data/` directory,
  not at the stage directory level. They are input references ("here is where
  my data comes from"), not topology declarations ("this is my child").

- **Self-documenting.** `ls _join_gather_rename/data/` shows exactly what
  feeds into the downstream stages. The directory name itself declares which
  streams converge. A human can read the tree and understand provenance
  without consulting the manifest.

- **Proper data-state.** The join node is a real materialized node with its
  own `join.json` artifact and its own fingerprint. It participates in the
  Merkle chain like every other node. If a parent is re-executed, the join
  node's recorded parent fingerprints become stale, and that staleness
  cascades to everything downstream.

- **Consistent.** Join nodes are always created when a stage has multiple
  parents, regardless of whether one parent is an ancestor of the other.
  The runtime never needs to reason about "implicit" vs "explicit" joins —
  every join is explicit.

==== Join Node Artifact

The `join.json` artifact records:

[source,json]
----
{
  "stage": "_join_gather_rename",
...
The content block records the parent references. The envelope carries the
fingerprint chain. The downstream stage (`harmonize`) lists the join node
as its parent in the session tree, and the join node's fingerprint
incorporates both parent fingerprints — so any upstream change propagates
through the join into all downstream stages.

==== Manifest vs Session Tree Divergence

The manifest declares 12 stages for the FedML workflow. The session tree
materializes those 12 stages plus N join nodes (one per multi-parent stage).
This is a well-defined, deterministic transform:

1. The parser reads the manifest and produces a `DAGDefinition` with the
   declared stages and edges.
2. The session materializer walks the DAG and identifies every node with
   multiple parents.
3. For each such node, it inserts a `_join_<parents>` node between the
   parents and the downstream stage.
4. The join node is synthetic (not in the manifest) but real in the session
   tree — it materializes, fingerprints, and participates in the chain.

The manifest remains clean and human-authored. The session tree reflects
the operational reality of how data flows converge.

==== Backend Generalization

The symlinks inside `data/` are input references, not topology. Their
mechanism adapts to the storage backend:

- **POSIX filesystem:** literal symlinks
- **Object storage:** small reference objects containing the target path
- **ZeroFS:** whatever linking primitive that system provides

The `StorageBackend.link_create()` method abstracts the mechanism. The
semantics are uniform: "this reference resolves to that parent's artifacts."
...
| `link(source, target)`
| Create a join input reference from source to target. Used inside
  topological join nodes' `data/` directories to reference parent artifact
  directories. On a POSIX filesystem, this is a symlink. On object storage,
  a small reference object containing the target path. The semantics are:
  "source points to target; reading source resolves to target's content."
...
| Real filesystem on the CalypsoServer sandbox. Enables persistent sessions
  across server restarts. Uses real symlinks for join node input references.
...
The complete FedML manifest with all 14 stages. The full file lives at
`src/dag/manifests/fedml.manifest.yaml`; the first four stages are shown
here in full, with remaining stages summarized:
...
stages:
  ...
  # code          → harmonize     produces: code.json           (accumulating)
  # train         → code          produces: train.json           (.local_pass)

  # Phase 4: Federation (8-step sequence)
  # federate-brief            → train                produces: briefing.json
  # federate-transcompile     → federate-brief       produces: transcompile.json
  # federate-containerize     → federate-transcompile produces: containerize.json
  # federate-publish-config   → federate-containerize produces: publish-config.json
  # federate-publish-execute  → federate-publish-config produces: publish-execute.json
  # federate-dispatch         → federate-publish-execute produces: dispatch.json
  # federate-execute          → federate-dispatch    produces: execute.json
  # federate-model-publish    → federate-execute     produces: model-publish.json (terminal)

  # See src/dag/manifests/fedml.manifest.yaml for the complete definition.
----

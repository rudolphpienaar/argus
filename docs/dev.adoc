...
== Testing Strategy (v1.3.0)

With v1.3.0, we introduced a formal testing harness to ensure logic stability as the prototype grows.

* **Framework:** Vitest (chosen for native ESM/TypeScript support and speed).
* **Scope:** Unit tests for pure logic extracted from the monolithic `argus.ts`.
* **Current Coverage:** Cost calculation engine (`src/core/logic/costs.ts`).

...
*   **Pattern**: Each stage module exports `onEnter()` and `onExit()` hooks. The main orchestrator (`argus.ts`) calls these hooks during transitions, inverting control and keeping the entry point clean.
